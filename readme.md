...
Aqua Demonstration App
- See the mgtsvr repo readme for information about the build/deploy pipeline
- This is a simple app that takes a 2d mask (black and white image) and creates a 3d STL file based on "extruding" the mask to a certain depth.
- The mgt folder contains build and deploy artifacts that can be triggered by the management instance.

- Manual steps to setup and run Aqua app locally
  - Create FastAPI app
  ...
  - Run the fastapi server - --reload automatically detects changes when in dev cycle
    - run from command-line
      - uvicorn app.service.main:app --reload --host 0.0.0.0 --port 8001
